...
volatile uint8_t sekundy_1 = 0;
volatile uint8_t sekundy_2 = 0;
volatile uint8_t minuty_1 = 0;
volatile uint8_t minuty_2 = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    if (htim->Instance == TIM9) // sprawdzam czy to timer 9 wywolal przerwanie
    {
        sekundy_1++;
        if (sekundy_1 >= 10) {
            sekundy_1 = 0;
