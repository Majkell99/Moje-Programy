...
volatile uint32_t adc_value_buffor[2];
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
    if(hadc->Instance == ADC1){ //przerwanie od ADC
        ...
        minuty_2 = (adc_value_buffor[0]/1000) % 10;
        minuty_1 = (adc_value_buffor[0]/100) % 10;
        sekundy_2 = (adc_value_buffor[0]/10) % 10;
        sekundy_1 = adc_value_buffor[0] % 10;          // Tutaj wyswietlany bedzie tylko kanal 1 ADC (czujnik optyczny)
        dane_gotowe = 1;
    }
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    ...
    {
        HAL_ADC_Start_DMA(&hadc1, adc_value_buffor, 2); //adc_value_buffor[0] - kanał 1 adc (czujnik optyczny)
    }                                                   //adc_value_buffor[1] - kanał 2 adc (czujnik temperatury)
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
        dane_gotowe = 0;
        sprintf(uart_buffor, "%lu,%lu\r\n", adc_value_buffor[0], adc_value_buffor[1]);
        if (HAL_UART_GetState(&huart2) == HAL_UART_STATE_READY) {
            HAL_UART_Transmit_DMA(&huart2, (uint8_t*)uart_buffor, strlen(uart_buffor));
        }
    }
    /* USER CODE BEGIN 3 */
  }
...
static void MX_TIM9_Init(void)
{
  ...
  htim9.Instance = TIM9;
  htim9.Init.Prescaler = 5000; // TO MA BYC 5000
  htim9.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim9.Init.Period = 199; // TO MA BYC 199
