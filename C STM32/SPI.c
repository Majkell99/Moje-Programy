...
{
    SPITx[0] = (0x80 | Address) << 8; // tutaj mamy 0000 0000 (1|Adres) a po << 8 mamy
    HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_RESET); // (1|Adres) 0000 0000
    HAL_SPI_TransmitReceive(&hspi1, SPITx, SPIRx, 1, 100);
    HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_SET); //Czy NSS to czujnik ktory sie wlacza?
    return( (uint8_t)(SPIRx[0] & 0xFF) );
}

void SPIWrite( uint8_t AddressAndAtributes, uint8_t Data  )
{
    SPITx[0] = (AddressAndAtributes << 8) + Data; //<< 8 oznacza ze najpierw przesuwam adres z prawych osmiu bitow do lewych
    HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_RESET); // i potem do prawych osmiu bitow zapisujemy Data
    HAL_SPI_TransmitReceive(&hspi1, SPITx, SPIRx, 1, 100); // najpier bylo 0000 0000 (Adres) a po zrobbeiniu <<8
    HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_SET); //mamy (Adres) 0000 0000
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    ...
    {
        x = (int8_t)SPIRead(0x29); // z kazdego pomiaru bierzemy 8 najstarszych bitow, a te 8 najmlodszych odcinami
        y = (int8_t)SPIRead(0x2B); // bo są najmniej istotne tak jak liczby po przecinku w normalnych liczbach
        z = (int8_t)SPIRead(0x2D); // 0x29|0x28 to jest caly odczyt x, 0x2B|0x2A to jest caly odczyt y
    }
}
...
int main(void)
{
  ...
  while (1)
  {
     ...
     int8_t z_raw = z;
         ...
         // 2. Przeliczamy na float (dodajemy 'f' po liczbie, żeby wymusić typ float)
     float x_f = (float)x_raw / 64.0f;
     float y_f = (float)y_raw / 64.0f;
     float z_f = (float)z_raw / 64.0f;

          // 3. Sprawdzamy, czy buffer nie jest za mały (zmień na 64 dla pewności)
     size = sprintf(buffor, "X:%.2f Y:%.2f Z:%.2f\r\n", x_f, y_f, z_f);
     //size = sprintf(buffor, "x = %f, y = %f, z = %f\n\r", x, y, z);
     HAL_UART_Transmit(&huart2, (uint8_t*)buffor, size, 100);  // czemu UART a nie DMA
     HAL_Delay(100);
    /* USER CODE END WHILE */
